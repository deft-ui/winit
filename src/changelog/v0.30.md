...
- Deprecate `EventLoopExtPumpEvents::pump_events`, use `EventLoopExtPumpEvents::pump_app_events`.
  ...
  use deft_winit::event::{Event, WindowEvent, DeviceEvent, DeviceId};
  use deft_winit::event_loop::{EventLoop, ActiveEventLoop};
  ...
  #[allow(deprecated)]
  let window = event_loop.create_window(Window::default_attributes()).unwrap();
  let state = State { window, counter: 0 };

  let _ = event_loop.run_app(state);
  ```
